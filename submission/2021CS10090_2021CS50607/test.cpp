...
public:
    ...
    void read_data(const std::string &filename)
    {
        ...
        if (file.is_open())
        {
            while (!file.eof())
            {
                ...
                if (missing_idx != -1)
                {
                    vals[missing_idx] = nValues[missing_idx] - 1;
    ...
    void write_network(const std::string &filename)
    {
        ...
        if (alarm.is_open() && solved_alarm.is_open())
        {
            ...
            alarm.close();
            solved_alarm.close();
            cout << "File has been modified and saved as "
                 << "solved_alarm.bif" << endl;
        }
        else
        {
...
void maximization(network &medical)
{
    for (int i = 0; i < medical.CPT.size(); i++)
    {
        ...
        medical.CPT[i] = vector<double>(medical.CPT[i].size(), 0.0);
        ...
        {
            if (denom[j % sz] == 0)
                medical.CPT[i][j] = 0.001;
            else
            {
                double s_factor = (denom[j % sz] / 1000.0);
                if (denom[j % sz] == 0.0)
                    s_factor = 0.0001;
                medical.CPT[i][j] += s_factor;
                medical.CPT[i][j] /= (denom[j % sz] + ((medical.nValues[i]) * s_factor));
            }
            if (medical.CPT[i][j] == 0.0)
                medical.CPT[i][j] = 0.0001;
            if (medical.CPT[i][j] == 1.0)
                medical.CPT[i][j] = 0.9999;
        }
    }
}
